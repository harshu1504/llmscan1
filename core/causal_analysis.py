...
class CausalMap:
    ...
    def to_feature_vector(self, max_layers: int = 40) -> np.ndarray:
        ...
        """
        # Token statistics (5 features as in paper)
        ...
        # Layer causal effects (fixed dimension)
...
class CausalScanner:
    ...
    def compute_token_causal_effects(
        ...
        CE_{x_i} = ||AS - AS'_{x_i}||
        where AS is original attention score, AS' is attention after token intervention.
        
        Paper uses tokens replaced with '-' one by one.
        """
        if selected_layers is None:
            total_layers = min(self.model_manager.get_total_layers(), 6)
            # Paper uses first, middle, last layers
            selected_layers = [0, total_layers // 2, total_layers - 1]
        
        if selected_heads is None:
            # Paper uses heads 1, 20, 40 for Llama-2-13b (scaled)
            selected_heads = [0, 15, 31]
        
        # Tokenize prompt
        tokens = self.model_manager.tokenizer(prompt, return_tensors="pt")
        input_ids = tokens.input_ids[0][:10]
        token_strings = [self.model_manager.tokenizer.decode([t]) for t in input_ids]
        ...
        # Intervene on each token (replace with '-' as per paper)
        for idx in range(len(input_ids)):
            # Token-level intervention (CORRECT WAY)
            ...
            for idx in range(input_ids.shape[1]):
                ...
                # Replace token with PAD token (safe intervention)
                modified_ids[0, idx] = self.model_manager.tokenizer.pad_token_id

                intervened_attentions = self._extract_attention_scores_from_ids(
                    modified_ids, selected_layers, selected_heads
                )
            
            # Compute Euclidean distance between attention score matrices
            distance = 0.0
            count = 0
            for layer in selected_layers:
                for head in selected_heads:
                    if layer in original_attentions and head in original_attentions[layer]:
                        ...
                        interv = np.array(intervened_attentions.get(layer, {}).get(head, orig))
                        distance += np.linalg.norm(orig - interv)
                        count += 1
            
            causal_effects[idx] = distance / max(count, 1)
    ...
    def _extract_attention_scores(
        ...
        cache_key = hashlib.sha256(f"{prompt}_{selected_layers}_{selected_heads}".encode()).hexdigest()
    ...
    def compute_layer_causal_effects(
        ...
        if baseline_output is None:
            baseline_output, _ = self.model_manager.run_inference(prompt)
        ...
        for layer_idx in range(total_layers):
            intervention = {layer_idx: {"type": "skip"}}
            
            try:
                modified_logit = self._get_first_token_logit_with_skip(prompt, layer_idx)
                ce = baseline_logit - modified_logit
                
                # Also compute semantic effect (innovation)
                modified_output, _ = self.model_manager.run_inference(prompt, intervention)
...
class AdaptiveLayerSelector:
    """
    Dynamically selects most informative layers for causal analysis.
    Innovation beyond paper's fixed first/middle/last approach.
    """
    ...
    def compute_layer_importance(self) -> Dict[int, float]:
        """Compute importance score for each layer based on causal effect variance."""
    ...
    def select_top_k_layers(self, k: int = 10) -> List[int]:
        """Select top-k most important layers for efficient scanning."""
...
class AttentionHeadPruner:
    """
    Identifies and prunes low-information attention heads for faster analysis.
    Innovation: optimizes the selective sampling from paper.
    """
    ...
    def compute_head_importance(
        ...
        """Compute importance score for each (layer, head) pair."""
...
class CausalMapStreamer:
    """
    Streams causal maps token-by-token for early misbehavior detection.
    Paper mentions detecting from first token - this implements that.
    """
    ...
    def stream_analysis(
        ...
        """
        Generate causal maps incrementally as tokens are processed.
        
        Yields: (token_position, cumulative_risk, causal_map_so_far)
        """
...
def visualize_causal_map(causal_map: CausalMap, figsize: Tuple[int, int] = (10, 6)):
    """
    Create visualization of causal map similar to Figure 2 in paper.
    """
